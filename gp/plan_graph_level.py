...
class PlanGraphLevel(object):
    ...
    def update_action_layer(self, previous_proposition_layer):
        ...
        for a in all_actions:
            if not previous_proposition_layer.all_preconds_in_layer(a):
                continue

            pre_actions = a.get_pre()
            flag = True
            for i in range(len(pre_actions)):
                for j in range(i + 1, len(pre_actions)):
                    if previous_proposition_layer.is_mutex(pre_actions[i], pre_actions[j]):
                        flag = False
                        break
            if flag:
    ...
    def update_proposition_layer(self):
        ...
        for a in current_layer_actions:
            for prop in a.get_add():
                prop_dict[prop].append(a)
    ...
    def update_mutex_proposition(self):
        ...
        for i in range(len(current_layer_propositions)):
            for j in range(len(current_layer_propositions)):
