...
class PlanningProblem:
    ...
    def get_successors(self, state):
        ...
        for a in self.actions:
            if a.all_preconds_in_list(state) and not a.is_noop():
                new_state = self.do_action(state, a)
                states.append((new_state, a, 1))
        return states
...
def level_sum(state, planning_problem):
    ...
    pg_init = PlanGraphLevel()
    pg_init.set_proposition_layer(prop_layer_init)
    sub_goal = set(planning_problem.goal) - pg_init.get_proposition_layer().get_propositions()
    level = 0
    cur_sum = 0
    graph = [pg_init]
    while planning_problem.goal_state_not_in_prop_layer(graph[level].get_proposition_layer().get_propositions()):
        ...
        pg_next = PlanGraphLevel()
        pg_next.expand_without_mutex(graph[level - 1])
        new_goal_prop_length = len(sub_goal & pg_next.get_proposition_layer().get_propositions())
        cur_sum += level * new_goal_prop_length
        sub_goal = sub_goal - pg_next.get_proposition_layer().get_propositions()
        graph.append(pg_next)
    return cur_sum
